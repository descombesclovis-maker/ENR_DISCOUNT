...
const STORAGE_KEY = "qeh_checkout_confirmation";

function getCheckoutCredentials(searchParams) {
  const urlSessionId = searchParams.get("session_id") || "";
  const urlToken = searchParams.get("confirmation_token") || "";
  ...
  if (urlSessionId && urlToken) {
    ...
    try {
      ...
      // La confirmation reste possible même si sessionStorage est indisponible.
    }
  ...
  try {
    ...
    if (
      ...
      Number(stored?.savedAt || 0) > Date.now() - 30 * 60 * 1000
    ) {
      return stored;
    }
  } catch {
    // Aucune donnée de confirmation récupérable.
  }

  return { sessionId: "", confirmationToken: "", savedAt: 0 };
}

export default function CheckoutSuccess() {
  const [searchParams] = useSearchParams();
  const [credentials] = useState(() => getCheckoutCredentials(searchParams));
  ...
  useEffect(() => {
    document.title = "Confirmation de commande | QEH OUTLET";

    if (sessionId && confirmationToken && window.location.search) {
  ...
  useEffect(() => {
    if (!sessionId || !confirmationToken) {
      setStatus("error");
      setErrorMessage("Les informations sécurisées de confirmation sont absentes ou ont expiré.");
      return;
    }
    ...
    async function checkPayment() {
      ...
      try {
        ...
        if (error) {
          let message = error.message || "Impossible de vérifier le paiement.";
        ...
        if (data?.payment_status === "paid") {
          if (!cartClearedRef.current) {
            ...
            clearCart();
            localStorage.removeItem("enr_cart");
          }

          try {
            sessionStorage.removeItem(STORAGE_KEY);
          } catch {
            // Sans effet sur la commande.
          }
        ...
        timerRef.current = setTimeout(checkPayment, POLLING_DELAY);
      } catch (error) {
  ...
  }, [sessionId, confirmationToken, clearCart]);
    ...
    return (
      <section className="max-w-3xl mx-auto px-5 sm:px-8 py-20">
        <div className="rounded-3xl border border-border bg-card p-7 sm:p-10 text-center">
          ...
          <p className="overline text-primary mb-2">Paiement confirmé</p>
          <h1 className="font-display font-black text-3xl sm:text-4xl">
          ...
          {order?.order_number && (
            <div className="rounded-2xl bg-secondary/50 mt-7 p-5">
              ...
              <p className="font-display font-bold text-xl mt-1">{order.order_number}</p>
              {order?.total !== undefined && (
                <p className="font-semibold mt-3">Total : {formatPrice(order.total)}</p>
              )}
            </div>
          )}
    ...
    return (
      <section className="max-w-3xl mx-auto px-5 sm:px-8 py-20">
        <div className="rounded-3xl border border-border bg-card p-7 sm:p-10 text-center">
          ...
          <h1 className="font-display font-black text-3xl">Paiement non confirmé</h1>
          <p className="text-muted-foreground mt-4">
    ...
    return (
      <section className="max-w-3xl mx-auto px-5 sm:px-8 py-20">
        <div className="rounded-3xl border border-border bg-card p-7 sm:p-10 text-center">
          ...
          <h1 className="font-display font-black text-3xl">Paiement en cours de confirmation</h1>
          <p className="text-muted-foreground mt-4">
  ...
  return (
    <section className="max-w-3xl mx-auto px-5 sm:px-8 py-20">
      <div className="rounded-3xl border border-border bg-card p-7 sm:p-10 text-center">
        <XCircle className="w-16 h-16 text-destructive mx-auto mb-6" />
        <h1 className="font-display font-black text-3xl">Impossible de confirmer la commande</h1>
        <p className="text-muted-foreground mt-4">
