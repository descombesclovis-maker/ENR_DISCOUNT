...
export default function AdminProducts() {
  ...
  async function toggleVisibility(product) {
    ...
    const nextVisibility = !product.is_active;
    setUpdatingProductId(product.id);
    setErrorMessage("");

    const { data, error } = await supabase.rpc(
      "qeh_set_product_visibility",
      {
        p_product_id: product.id,
        p_is_active: nextVisibility,
      }
    );
    ...
    if (error) {
      ...
      setErrorMessage(
        normalizedMessage.includes("qeh_set_product_visibility") ||
        normalizedMessage.includes("schema cache")
          ? "Le correctif RLS QEH OUTLET n’est pas encore installé dans Supabase. Exécutez le fichier SQL fourni, puis réessayez."
          : error.message
      );
    } else if (data !== true) {
      setErrorMessage(
        "La visibilité du produit n’a pas été modifiée. Actualisez la page puis réessayez."
      );
    } else {
      setProducts((current) =>
        current.map((item) =>
          ...
          {loading ? (
            ...
            <div className="grid min-h-[340px] place-items-center p-6 text-center">
              <div>
                ...
                <p className="mt-4 font-black">Aucun produit trouvé</p>
              ...
              {filteredProducts.map((product, index) => {
                ...
                return (
                  <motion.article
                    ...
                    key={product.id}
                    className="relative isolate overflow-hidden rounded-3xl border border-slate-200 bg-white transition hover:-translate-y-1 hover:border-orange-200 hover:shadow-xl"
                  >
                    <div
                      className="relative grid place-items-center overflow-hidden bg-slate-50 p-4"
                      style={{ height: "12rem", minHeight: "12rem", maxHeight: "12rem" }}
                    >
                      {image ? (
                        <img
                          src={image}
                        ...
                        <Boxes className="h-16 w-16 text-slate-300" />
                      )}

                      <span
                        className={`absolute right-3 top-3 rounded-full px-3 py-1.5 text-xs font-black ${
                          product.is_active
                      ...
                      {product.requires_pallet ? (
                        <span className="absolute bottom-3 left-3 rounded-full bg-[#050b16] px-3 py-1 text-[10px] font-black uppercase text-white">
                          Palette
                        </span>
                      ) : null}
                    </div>
                    ...
                    <div className="relative z-10 bg-white p-5">
                      <p className="text-xs font-black uppercase tracking-[.14em] text-[#ff5a00]">
                        {product.brand || product.categories?.name || "QEH Sélection"}
                      </p>
                      <h2 className="mt-2 line-clamp-2 min-h-[56px] text-xl font-black text-slate-950">
                        {product.name}
                      </h2>
                      <p className="mt-2 text-xs font-semibold text-slate-500">
                        {product.reference || product.sku || "Sans référence"}
                      </p>

                      <div className="mt-5 grid grid-cols-3 gap-2 text-center">
                        <div className="rounded-xl bg-slate-50 p-2">
                          <p className="text-[10px] font-bold text-slate-400">PRIX</p>
                          <p className="mt-1 font-black">{money(displayedPrice)}</p>
                        </div>
                        <div className="rounded-xl bg-slate-50 p-2">
                        ...
                        <div className="rounded-xl bg-slate-50 p-2">
                          <p className="text-[10px] font-bold text-slate-400">VARIANTES</p>
                          <p className="mt-1 font-black">{variants.length}</p>
                          <p className="text-[9px] text-slate-400">{variantStock} unités</p>
                        </div>
                      </div>
                      ...
                      <div className="relative z-20 mt-5 flex gap-2">
                        <Link
                          ...
                          className="inline-flex min-h-11 flex-1 items-center justify-center gap-2 rounded-xl bg-[#050b16] font-black text-white hover:bg-[#ff5a00]"
                        >
                        ...
                        <button
                          ...
                          className="grid h-11 w-11 place-items-center rounded-xl border border-slate-200 text-slate-600 disabled:cursor-wait disabled:opacity-50"
                        ...
                        <button
                          ...
                          className="grid h-11 w-11 place-items-center rounded-xl border border-red-100 text-red-500"
