...
const trackingServices = {
  ...
  mondial: {
    ...
    description: "Livraison en Point Relais ou à domicile",
...
function PublicTrackingView({ token }) {
    ...
    const load = async () => {
      ...
      try {
        const { data, error } = await supabase.functions.invoke("qeh-order-tracking", {
          body: { action: "public_tracking", tracking_token: token },
        });
        if (error || data?.error) throw new Error(data?.error || error?.message || "Suivi indisponible.");
        if (active) setTracking(data);
      } catch (error) {
        console.error("Impossible de charger le suivi QEH :", error);
        if (active) setErrorMessage(error?.message || "Impossible de charger le suivi.");
      } finally {
    ...
    return (
      <div className="min-h-[65vh] grid place-items-center bg-slate-50 px-5">
        <div className="max-w-xl rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-3 text-slate-500">{errorMessage || "Ce lien de suivi n’est pas valide."}</p>
        </div>
      </div>
    );
  }
  ...
  const activeIndex = order.fulfillment_status === "delivered" ? 2 : order.fulfillment_status === "shipped" ? 1 : 0;
    ...
    <div className="min-h-screen bg-slate-50">
      <section className="relative overflow-hidden bg-[#020714] text-white">
        ...
        <div className="relative max-w-6xl mx-auto px-5 sm:px-8 py-14 sm:py-20">
          <p className="text-xs font-black uppercase tracking-[0.24em] text-[#ff5a00]">QEH OUTLET · SUIVI DE LIVRAISON</p>
          <h1 className="font-display font-black text-4xl sm:text-5xl tracking-tight mt-3">Commande #{order.order_number}</h1>
      ...
      <main className="max-w-6xl mx-auto px-5 sm:px-8 py-10 sm:py-14">
        <section className="rounded-[32px] border border-slate-200 bg-white p-6 sm:p-8 shadow-[0_20px_55px_rgba(2,7,20,0.07)]">
          <div className="grid md:grid-cols-3 gap-5">
            {statusSteps.map((step, index) => {
              ...
              return (
                <div key={step.key} className={`rounded-3xl border p-5 ${reached ? "border-[#ff5a00]/25 bg-[#ff5a00]/[.04]" : "border-slate-200 bg-slate-50"}`}>
                  <div className={`w-11 h-11 rounded-2xl grid place-items-center ${reached ? "bg-[#ff5a00] text-white" : "bg-slate-200 text-slate-400"}`}>
                    <Icon className="w-5 h-5" />
                  </div>
                  <h2 className="font-display font-black text-lg text-slate-950 mt-4">{step.label}</h2>
                  <p className="text-sm text-slate-500 mt-2">{formatDate(event?.occurred_at)}</p>
                  {event?.message && <p className="text-sm text-slate-600 leading-relaxed mt-3">{event.message}</p>}
                </div>
              );
            })}
          </div>
        ...
        <div className="grid lg:grid-cols-[1fr_320px] gap-8 mt-8 items-start">
          <section className="rounded-[32px] border border-slate-200 bg-white p-6 sm:p-8">
            <h2 className="font-display font-black text-2xl text-slate-950">Informations de livraison</h2>
            <div className="grid sm:grid-cols-2 gap-4 mt-6">
              <div className="rounded-2xl bg-slate-50 border border-slate-200 p-5">
                <p className="text-xs font-black uppercase tracking-[.14em] text-slate-400">Transporteur</p>
                <p className="font-black text-slate-950 mt-2">{order.carrier || "En cours d’attribution"}</p>
              </div>
              <div className="rounded-2xl bg-slate-50 border border-slate-200 p-5">
                <p className="text-xs font-black uppercase tracking-[.14em] text-slate-400">Numéro de suivi</p>
                <p className="font-black text-slate-950 mt-2 break-all">{order.tracking_number || "Disponible à l’expédition"}</p>
              </div>
              <div className="rounded-2xl bg-slate-50 border border-slate-200 p-5 sm:col-span-2">
                <p className="text-xs font-black uppercase tracking-[.14em] text-slate-400">Livraison estimée</p>
                <p className="font-black text-slate-950 mt-2">
                  {order.estimated_delivery_date
                    ? new Intl.DateTimeFormat("fr-FR", { dateStyle: "long" }).format(new Date(`${order.estimated_delivery_date}T12:00:00`))
          ...
          <aside className="rounded-3xl border border-[#0b5ca8]/20 bg-[#020714] p-6 text-white">
            ...
            <h2 className="font-display font-black text-xl mt-5">Suivi QEH sécurisé</h2>
...
export default function Tracking() {
  ...
  useEffect(() => {
    document.title = "Suivi de commande | QEH OUTLET";
  }, []);

  if (token) return <PublicTrackingView token={token} />;
  ...
  const handleTracking = async (event) => {
    ...
    toast.success("Le suivi officiel est ouvert.", {
      description: copied ? "Votre numéro a été copié : collez-le chez le transporteur." : undefined,
    });
  };
    ...
    <div className="min-h-screen bg-slate-50">
      <section className="relative overflow-hidden bg-[#020714]">
        <div className="relative max-w-7xl mx-auto px-5 sm:px-8 py-14 sm:py-20">
          ...
          <p className="text-xs font-black uppercase tracking-[0.24em] text-[#ff5a00] mt-7">QEH OUTLET</p>
          <h1 className="font-display font-black text-4xl sm:text-5xl text-white tracking-tight mt-3">Suivre une commande</h1>
          <p className="max-w-2xl text-white/60 leading-relaxed mt-5">
            Si QEH OUTLET vous a envoyé un bouton « Suivre ma livraison », utilisez directement ce lien. Sinon, vous pouvez encore ouvrir le suivi officiel du transporteur ci-dessous.
          </p>
        </div>
      </section>

      <main className="max-w-5xl mx-auto px-5 sm:px-8 py-10 sm:py-14">
        <form onSubmit={handleTracking} className="rounded-3xl border border-slate-200 bg-white p-6 sm:p-8 shadow-sm space-y-7">
          <div>
            <p className="text-sm font-bold text-slate-950 mb-3">Transporteur</p>
            <div className="grid sm:grid-cols-3 gap-3">
              {Object.entries(trackingServices).map(([key, service]) => (
                <button
                  ...
                  className={`rounded-2xl border p-4 text-left transition ${selectedCarrier === key ? "border-[#ff5a00] bg-[#ff5a00]/5" : "border-slate-200"}`}
                >
                  <p className="font-black text-slate-950">{service.shortName}</p>
          ...
          <div>
            <label htmlFor="tracking-number" className="block text-sm font-bold text-slate-950 mb-2">Numéro de suivi</label>
          ...
          <div className="rounded-2xl border border-[#0b5ca8]/20 bg-[#0b5ca8]/5 p-5 flex items-start gap-3">
            ...
            <p className="text-sm text-slate-600">Le bouton ouvre le site officiel de {selectedService.name}. QEH OUTLET ne modifie pas les informations fournies par le transporteur.</p>
          </div>
          ...
          >
            Suivre avec {selectedService.name}
            <ExternalLink className="w-4 h-4" />
          </button>
        </form>
      </main>
